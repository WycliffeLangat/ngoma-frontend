...
const configs = {
  ...
  certifications: { title: "Certifications", endpoint: "/certifications/", columns: [{key:"title",label:"Release"},{key:"artist",label:"Artist"},{key:"level",label:"Level"},{key:"total_points",label:"Points"}], form: [{name:"release",label:"Release ID",type:"number"},{name:"level",label:"Level",type:"select",readOnly:true,help:"Auto-filled from Points using the active certification rules.",options:[{value:"gold",label:"Pulse"},{value:"platinum",label:"Wave"},{value:"diamond",label:"Legacy"}]},{name:"total_points",label:"Points",type:"number",help:"If points are below the lowest active threshold, the CMS automatically hides the certification."},{name:"is_hidden",label:"Hidden from app",type:"checkbox"},{name:"certification_date",label:"Certification date",type:"date"},{name:"previous_level",label:"Previous level"},{name:"notes",label:"Notes",type:"textarea"}] },
  "certification-rules": { title: "Certification Rules", endpoint: "/certification-rules/", columns: [{key:"label",label:"Level",render:(r)=>certificationLabel(r.level || r.label)},{key:"threshold",label:"Points threshold"},{key:"active",label:"Active"},{key:"updated_at",label:"Updated",render:(r)=>new Date(r.updated_at).toLocaleDateString()}], form: [{name:"level",label:"Level",type:"select",options:[{value:"gold",label:"Pulse"},{value:"platinum",label:"Wave"},{value:"diamond",label:"Legacy"}]},{name:"threshold",label:"Points threshold",type:"number"},{name:"active",label:"Active",type:"checkbox"}] },
...
export default function ResourcePage({ type, searchJump, user, onNavigate }) {
  ...
  async function save(form) {
    ...
    if (certificationCorrection?.hidden) {
      ...
      showFlash(`Certification level auto-corrected to ${certificationLabel(certificationCorrection.eligibleLevel)}.`);
    }
  }
    ...
    <section>
      ...
      {ALPHA_TYPES.has(type) && (
        <div className="cms-alpha-bar" style={{ background: "#f7f5f0", border: "1px solid #e8e0cc", borderRadius: 8, padding: "8px 12px", margin: "0 0 12px", display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
          ...
          <button
            type="button"
            style={{ minWidth: 40, height: 28, padding: "0 10px", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", background: !alphaFilter ? "#bf870e" : "#fff", color: !alphaFilter ? "#fff" : "#000000", boxShadow: !alphaFilter ? "0 1px 4px rgba(201,122,18,.35)" : "0 1px 2px rgba(0,0,0,.08)" }}
          ...
          {ALPHABET.map(letter => (
            <button
              key={letter}
              type="button"
              style={{ width: 28, height: 28, border: "none", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", background: alphaFilter === letter ? "#bf870e" : "#fff", color: alphaFilter === letter ? "#fff" : "#000000", boxShadow: alphaFilter === letter ? "0 1px 4px rgba(201,122,18,.35)" : "0 1px 2px rgba(0,0,0,.08)" }}
              onClick={() => setAlphaFilter(alphaFilter === letter ? "" : letter)}
            >{letter}</button>
          ))}
          {(alphaFilter || search) && (
            <span style={{ marginLeft: 6, fontSize: 11, color: "#bf870e", fontWeight: 600 }}>
              {alphaFilter && search ? `${alphaFilter}… + "${search}"` : alphaFilter ? `Showing: ${alphaFilter}` : `"${search}"`}
              {alphaFilter && (
                <button type="button" onClick={() => setAlphaFilter("")} style={{ border: "none", background: "none", color: "#bf870e", cursor: "pointer", fontSize: 12, padding: "0 0 0 6px", fontWeight: 700 }}>✕ letter</button>
              )}
            </span>
          )}
        </div>
                    ...
                    {r.certifications?.length > 0 && (
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
                        ...
                        {r.certifications.map((c, i) => {
                          ...
                          const meta = CERTIFICATION_BRANDING[lvl];
                          const [bg, fg] = meta ? [`${meta.color}22`, meta.textColor] : ["#f0f0f0", "#555"];
                          return <span key={i} style={{ fontSize: 11, fontWeight: 700, background: bg, color: fg, border: `1px solid ${fg}`, borderRadius: 4, padding: "2px 9px", textTransform: "capitalize" }}><CertificationIcon level={lvl} /> {certificationLabel(lvl)}</span>;
                        })}
                      </div>
                    )}
