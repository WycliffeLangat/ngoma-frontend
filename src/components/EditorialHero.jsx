...
export default function EditorialHero({ eyebrow, title, description, metric, metricLabel, entry, pool, actions, onOpen, isArtist = false, showVisual = true }) {
  ...
  return (
    <section className={`v2-editorial-hero${showVisual ? "" : " v2-editorial-hero-copy-only"}`}>
      ...
      {showVisual && <aside className="v2-editorial-visual" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false); }}>
