        ...
        <>
          <div style={{ position: "absolute", top: 320, left: padX, right: padX, display: "flex", gap: 20, zIndex: 1 }}>
            {[[profile1, GOLD], [profile2, BLUE]].map(([p, color], i) => (
              <div key={i} style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: 14, borderRadius: 24, background: t.rowBg, borderLeft: `4px solid ${color}` }}>
                {p.cover_image || p.image ? (
                  <img src={resolveMediaUrl(p.cover_image || p.image)} alt="" style={{ width: 128, height: 128, borderRadius: chartType === "artists" ? 50 : 10, objectFit: "cover" }} />
                ) : (
