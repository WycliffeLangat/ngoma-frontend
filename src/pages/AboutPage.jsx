    ...
    <div style={{padding:PAD,background:isDark?"#050505":"#FFF",minHeight:"60vh",boxSizing:"border-box",overflow:"hidden"}}>
      ...
      <div className="anl-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}>
        ...
        <div style={card()}>
          ...
          <div style={{display:"grid",gap:"8px"}}>{CERTIFICATION_LEVELS.map(level=><div key={level.level} style={{display:"grid",gridTemplateColumns:"34px minmax(0,1fr) auto",gap:"9px",alignItems:"center",padding:"9px 10px",borderRadius:"11px",background:`${level.color}0B`,border:`1px solid ${level.color}25`}}><span style={{fontSize:"22px",textAlign:"center"}}>{level.icon}</span><strong style={{fontFamily:F,fontSize:"14px",color:isDark ? level.color : level.textColor,display:"inline-flex",alignItems:"center",gap:"6px"}}>{level.label}{info(level.label, `${level.label} is awarded when a song or album reaches at least ${level.pts.toLocaleString()} lifetime Combined chart points.`, [], 13)}</strong><span style={{fontFamily:F,fontSize:"13px",fontWeight:800,color:textPrimary,display:"inline-flex",alignItems:"center",gap:"6px"}}>{level.pts.toLocaleString()}+ pts{info(`${level.label} Points`, "This is the lifetime Combined points threshold required for the award tier.", [], 13)}</span></div>)}</div>
        </div>
        <div style={card()}>
