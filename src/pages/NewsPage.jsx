          ...
          <div className="news-grid" style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,minmax(0,1fr))",gap:isMobile?"16px":"20px"}}>
            {NEWS.map((n,i)=>{
              ...
              <div key={n.id}
                ...
                style={{...card({cursor:"pointer",padding:isMobile?"16px":"22px",transition:"transform 200ms ease, box-shadow 200ms ease",gridColumn:isFeatured?"1 / -1":"auto",background:isDark?"#0F120F":"#FFF",borderColor:isDark?"#2F352F":"#E8E5DC",boxShadow:isDark?"none":"0 2px 8px rgba(0,0,0,0.04)"}),...(i===0?{background:isDark?"#111411":"#FAF5EA",borderColor:GOLD+"44",borderLeft:"3px solid "+GOLD}:{})}}
                ...
                <div style={{display:"flex",gap:isMobile?"10px":"14px",alignItems:"flex-start",minWidth:0}}>
                  {i===0&&n.emoji&&<div style={{fontSize:isMobile?"27px":"38px",flexShrink:0,marginTop:"2px"}}>{n.emoji}</div>}
