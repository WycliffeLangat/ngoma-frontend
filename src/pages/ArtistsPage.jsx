export default function ArtistsPage({ ctx }) {
  ...
  const ArtistArtwork = ({ artist, size = 54, compact = false, style = {} }) => {
    ...
    const fallbackLabel = String(name || "A")
      .split(/\s+/)
      .filter(Boolean)
      .slice(0, 2)
      .map((part) => part[0])
      ...
      return (
        <div
          style={{
            ...
            boxShadow: isDark ? "0 8px 22px rgba(0,0,0,0.24)" : "0 8px 20px rgba(0,0,0,0.08)",
            ...style,
          }}
        >
          <img
    ...
    return (
      <div
        style={{
          ...
          borderRadius: compact ? "14px" : "16px",
          border: "1px solid " + (isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)"),
          background: isDark ? "linear-gradient(135deg, rgba(184,134,11,0.28), rgba(21,101,192,0.18))" : "linear-gradient(135deg, rgba(184,134,11,0.12), rgba(21,101,192,0.08))",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          color: isDark ? "#F6F3EA" : "#1F241F",
          fontFamily: F,
          fontSize: size >= 48 ? "16px" : "13px",
          fontWeight: 900,
...
<div style={{padding:PAD,background:isDark?"#050505":"#FFF",minHeight:"60vh",boxSizing:"border-box",overflow:"hidden"}}>
          ...
          <div style={{...card(),padding:isMobile?"18px":"22px",marginBottom:"22px",background:isDark?"#111411":"#FAFAF8"}}>
            ...
            <div className="anl-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:isMobile?"10px":"12px",marginBottom:isMobile?"12px":"14px"}}>
              {[{d:cmp1,c:GOLD},{d:cmp2,c:isDark?"#FFFFFF":"#000000"}].map(({d,c},i)=>(
                <button key={i} type="button" onClick={()=>openArtistDetails(d.n)} style={{padding:isMobile?"13px":"15px",background:isDark?(i===0?"rgba(184,134,11,0.16)":"rgba(255,255,255,0.06)"):c+"0D",borderRadius:"10px",border:isDark?"1px solid "+c+"55":"none",borderLeft:"3px solid "+c,cursor:"pointer",minWidth:0,textAlign:"left"}}>
                  <div style={{display:"flex",alignItems:"center",gap:"8px",minWidth:0}}>
                    ...
                    <div style={{fontFamily:SF,fontSize:isMobile?"15px":"16px",fontWeight:850,lineHeight:1.2,whiteSpace:"normal",overflowWrap:"anywhere",color:isDark?"#FFFFFF":"#000000"}}>{d.n}</div>
                  </div>
                </button>
              ))}
            </div>
            <div style={{width:"100%",maxWidth:isMobile?"360px":"none",margin:"0 auto",border:"1px solid "+(isDark?"#2F352F":"#E4E1D8"),borderRadius:"12px",overflow:"hidden",background:isDark?"#0F120F":"#FFF",boxShadow:isDark?"none":"0 8px 24px rgba(31,36,31,0.05)"}}>
              <div style={{display:"grid",gridTemplateColumns:isMobile?"minmax(76px,1fr) minmax(100px,0.9fr) minmax(76px,1fr)":"minmax(130px,1fr) minmax(150px,0.8fr) minmax(130px,1fr)",gap:"8px",alignItems:"center",padding:isMobile?"10px 9px":"12px 16px",background:"#1F241F",color:"#FFF"}}>
                ...
                <div style={{fontFamily:F,fontSize:"9px",fontWeight:900,letterSpacing:"1.4px",textAlign:"center",textTransform:"uppercase",color:isDark?"#FFFFFF":"#000000"}}>Metric</div>
          ...
          {isMobile ? (
            <div style={{display:"grid",gap:"10px"}}>
              {artists.slice(0,50).map((a,i)=>{
                ...
                return(
                  <div key={rowKey} style={{padding:"15px 16px",border:"1px solid "+(isDark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.08)"),borderRadius:"16px",background:isDark?"#0F1110":"#FFF",boxShadow:expanded?"inset 4px 0 0 #C97A12, 0 8px 22px rgba(0,0,0,0.045)":"0 2px 10px rgba(0,0,0,0.025)"}}>
                    <div onClick={()=>toggleArtistRow(rowKey)} role="button" aria-expanded={expanded} style={{display:"grid",gridTemplateColumns:"34px 42px minmax(0,1fr) 38px",gap:"10px",alignItems:"center",cursor:"pointer",minWidth:0}}>
                      <div style={{fontSize:i<3?"28px":"24px",fontWeight:950,lineHeight:1,color:i<3?MEDALS[i]:(isDark?"#FFFFFF":"#000000"),textAlign:"center",fontFamily:F}}>{i+1}</div>
                      <ArtistArtwork artist={a} size={54} />
                      <div style={{minWidth:0}}>
                        <button type="button" onClick={(event)=>{event.stopPropagation();openArtistDetails(a.n);}} style={{display:"block",width:"100%",border:0,background:"transparent",padding:0,margin:0,textAlign:"left",fontFamily:SF,fontSize:"15.5px",fontWeight:850,lineHeight:1.2,color:isDark?"#FFFFFF":"#000000",whiteSpace:"normal",overflowWrap:"anywhere",cursor:"pointer"}}>{a.n}</button>
            ...
            {artists.slice(0,50).map((a,i)=>{const trend=artistTrendFor(a);return(
              <div key={a.n} className="ngoma-artist-row" style={{display:"grid",gridTemplateColumns:"44px 38px minmax(0,1fr) 70px 126px",gap:"12px",padding:"12px",borderBottom:"1px solid #F2F2EE",alignItems:"center",minWidth:0}}
                ...
                <div style={{fontSize:i<3?"20px":"14px",fontWeight:900,color:i<3?MEDALS[i]:(isDark?"#FFFFFF":"#000000"),textAlign:"center",fontFamily:F,letterSpacing:"-0.3px"}}>{i+1}</div>
                <ArtistArtwork artist={a} size={46} compact />
                <div style={{minWidth:0}}><button type="button" onClick={()=>openArtistDetails(a.n)} style={{border:0,background:"transparent",padding:0,fontFamily:SF,fontSize:"15.5px",fontWeight:850,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",lineHeight:1.15,cursor:"pointer",maxWidth:"100%",textAlign:"left"}}>{a.n}</button><div style={{fontSize:"12px",color:isDark?"#FFFFFF":"#000000",fontFamily:F,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginTop:"4px",lineHeight:1.35}}>{a.t} {a.t===1?"entry":"entries"} · Artist peak: #{a.pk} · {a.m} {a.m===1?"month":"months"}</div></div>
